...
export async function POST(request, context) {
  ...
  try {
    ...
    if (!["pending", "under_consideration", "rejected"].includes(bid.status)) {
      return NextResponse.json(
        { error: "This bid cannot be accepted in its current status" },
        { status: 400 },
      );
    }
    ...
    const order = await Order.create({
      ...
      status: "confirmed",
      cancellationWindowExpiresAt: new Date(Date.now() + 48 * 60 * 60 * 1000),
    ...
    if (isStripeEnabled() && paymentIntentId) {
      try {
        ...
        console.error("Bid acceptance payment capture failed:", stripeErr.message);
      }
    }

    // Notify winning manufacturer
    await notify.orderConfirmedFromBid(
      bid.manufacturerId,
      order._id,
      order.orderNumber,
    );
