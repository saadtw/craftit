                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-6 text-xs text-white/40 mb-4">
                            <span>
                              Qty: {rfq.customOrderId?.quantity || "—"}
                            </span>
                            {rfq.customOrderId?.budget && (
                              <span className="text-emerald-400 font-bold">
                                Budget {formatPKR(rfq.customOrderId.budget)}
                              </span>
                            )}
              ...
              {loading ? (
                ...
                <div className="py-32 text-center bg-white/[0.02] rounded-[3rem] border-2 border-dashed border-white/5">
                  <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/20">
                    No matching inquiries in this sector
                  </p>
                </div>
              ) : (
                rfqs.map((rfq) => (
                    ...
                    <div className="flex-1 w-full">
                      <div className="flex justify-between items-start mb-4">
                        <div>
                          <div className="flex flex-wrap items-center gap-3 mb-2">
                            ...
                            <span
                              className={`px-2 py-0.5 text-[8px] font-black uppercase tracking-widest rounded-md border ${STATUS_STYLES[rfq.status] || STATUS_STYLES.active}`}
                            >
                              {rfq.status}
                            </span>
                      ...
                      {rfq.matchReasons && rfq.matchReasons.length > 0 && (
                        <div className="flex flex-wrap gap-2 mb-8">
                          {rfq.matchReasons.map((reason, idx) => (
                            <span
                              key={idx}
                              className="px-2.5 py-1 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-md text-[9px] font-black uppercase tracking-widest"
                            >
                              {reason}
                            </span>
                          ))}
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-8 pt-6 border-t border-white/5">
                        ...
                        <div className="flex justify-end items-end gap-3">
                          <Link
                            href={`/manufacturer/rfqs/${rfq._id}`}
                            className="px-5 py-2.5 bg-white/5 border border-white/10 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-white/10 transition-all"
                          >
                            Review
                          </Link>
                          <Link
                            href={`/manufacturer/rfqs/${rfq._id}/bid`}
                            className="px-6 py-2.5 bg-purple-600 text-white rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-purple-500 shadow-lg transition-all"
                          >
                            Bid Now
                          </Link>
                        </div>
