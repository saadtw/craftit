...
export async function sendOtpEmail({ to, name, otp }) {
  const subject = "Your Craftit verification code";
  const intro = `Hi ${name || "there"}, your email verification code is:`;
  const html = `
  <div style="font-family:Arial,sans-serif;line-height:1.5;color:#111827;max-width:600px;margin:0 auto;padding:24px">
    <h2 style="margin:0 0 12px">Verify your email</h2>
    <p style="margin:0 0 16px">${intro}</p>
    <div style="font-size:36px;font-weight:800;letter-spacing:8px;color:#1d4ed8;background:#eff6ff;padding:16px 24px;border-radius:8px;text-align:center;margin-bottom:16px">${otp}</div>
    <p style="margin:0 0 12px">This code expires in <strong>15 minutes</strong>.</p>
    <p style="font-size:12px;color:#6b7280;margin-top:24px">If you did not create a Craftit account, you can safely ignore this email.</p>
  </div>`;
