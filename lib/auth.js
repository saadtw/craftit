      ...
      async authorize(credentials) {
        try {
          ...
          const user = await User.findOne({ email: normalizedEmail }).select(
            "name role isActive isEmailVerified twoFactorEnabled verificationStatus businessName sessionVersion suspendedAt suspendedUntil supabaseId +twoFactorCodeToken +twoFactorCodeExpires"
          );
    ...
    async jwt({ token, user }) {
      ...
      try {
        ...
        const hasVersionMismatch =
          !!dbUser && (dbUser.sessionVersion ?? 0) !== (token.sessionVersion ?? 0);
