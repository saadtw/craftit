...
export const authOptions = {
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        try {
          ...
          // OAuth users can log in with email/password only after they set a local password.
          if (!user.password) {
            throw new Error(
  ...
  callbacks: {
      ...
      try {
        ...
        if (!dbUser) {
          ...
          if (!hasProvider) {
            ...
            // Keep the existing authMethod; credentials login is allowed as long as a local password exists.
          }
        ...
        user.requiresPasswordSetup = dbUser.requiresPasswordSetup ?? false;
    ...
    async jwt({ token, user }) {
      if (user) {
        if (!user.role && user.email) {
          ...
          if (dbUser) {
            ...
            token.requiresPasswordSetup = dbUser.requiresPasswordSetup ?? false;
        ...
        token.requiresPasswordSetup = user.requiresPasswordSetup ?? false;
        token.sessionInvalid = false;
        token.lastValidatedAt = Date.now();
        return token;
      }
      ...
      try {
        ...
        const dbUser = await User.findById(token.id).select(
          "_id role verificationStatus businessName isActive suspendedAt suspendedUntil sessionVersion isEmailVerified twoFactorEnabled requiresPasswordSetup",
        );
    ...
    async session({ session, token }) {
      ...
      session.user.requiresPasswordSetup = token.requiresPasswordSetup ?? false;
